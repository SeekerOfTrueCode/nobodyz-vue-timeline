<template>
  <div ref="tooltip-ref" :style="{ ...tooltipPositionStyle, 'pointer-events': 'none'}">
...
export default class TimelineTooltip extends Vue {
    ...
    private created() {
      (this.$parent as any).timelineRowRectListeners.mouseenter = this.showTooltip;
      (this.$parent as any).timelineRowRectListeners.mouseleave = this.hideTooltip;
    }
    ...
    private get relativePos() {
      if (this.tooltip) {
        return calculateRelativePos(this.tooltipRef, this.tooltip?.target as HTMLElement);
      } else {
        return {
          x: 0,
          y: 0
    ...
    private get tooltipPositionStyle() {
      if (this.relativePos.x && this.relativePos.y) {
        const targetRect = (this.tooltip?.target as HTMLElement).getBoundingClientRect();
        const rectCenter = {
